...
function sendSvgToParent() {
  console.log('sendSvgToParent called from all.js.');
  ...
  svgNodes.forEach(function(item){
    ...
    // Adds node data to the JSON array so user can retrieve and edit.
    var jsonified = toJSON(item);
    jsonNodes.push(jsonified);

    // Adds node data to SVG string for pre-rendered SVG image.
  ...
  window.parent.postMessage(
    JSON.stringify({'SVG_NODES': jsonNodes, 'SVG_TEXT': text}), "*");
}
...
function onAllLoaded() {
    ...
    downloadBtn.addEventListener("click", sendSvgToParent, false);
